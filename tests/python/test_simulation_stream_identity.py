"""The facade default mixture is [physical@0.01, directed@0.99].

The compiled default biasing must reproduce the fixed channel order and the
...
def test_default_builder_is_physical_first_point_99():
    ...
    # Physical channel FIRST, directed SECOND (required order).
...
def test_default_fraction_is_point_99_not_point_98():
    """Omitting fraction uses 0.99, producing the 0.01/0.99 split."""
    sig = _two_body_signature()
    xs = interactions.DummyCrossSection()
...
def test_default_matches_hand_built_legacy_mixture():
    ...
    # Hand build for comparison: physical appended first, directed second.
...
def test_bias_targets_default_fraction_is_point_99():
    ...
    A user omitting fraction on bias_targets must get the 0.01/0.99
