...
class OverrideMassDecay(siren.DecayModel):
    """Daughter masses supplied by a SecondaryMasses override -- the route
    BSM daughters absent from the C++ mass map take (VectorPortal models);
    the sampler bridge must consult the override, not the map."""
...
def _legacy_decay_injector(model, events=4, seed=101):
    """A legacy attribute-form Injector (interactions only, no phase spaces)
    around a models.py decay as the primary process. A bare vacuum detector
    suffices: decays need no material targets. Because no phase space is
    registered, GenerateEvent routes through SampleMatchingFinalState, whose
    record arrives at SampleFinalState with unsized secondary storage."""
    return siren.injection.Injector(
        number_of_events=events,
        detector_model=siren.detector.DetectorModel(),
        seed=seed,
        primary_type=P.N4,
        primary_interactions=[model],
        primary_injection_distributions=[
            siren.distributions.PrimaryMass(0.02),
            siren.distributions.Monoenergetic(0.05),
            siren.distributions.FixedDirection(siren.math.Vector3D(0, 0, 1)),
            siren.distributions.PointSourcePositionDistribution(
                siren.math.Vector3D(0, 0, 0), 300.0),
        ],
    )
...
def test_legacy_interface_drives_default_sampler():
    ...
    inj = _legacy_decay_injector(IsoDecay())
    trees = inj.generate(3, on_shortfall="raise")
    assert len(trees) == 3
    for tree in trees:
        (datum,) = list(tree.tree)
        rec = datum.record
        assert len(rec.secondary_momenta) == 2
        p_in = list(rec.primary_momentum)
        p_out = [sum(p[k] for p in rec.secondary_momenta) for k in range(4)]
        assert p_out == pytest.approx(p_in, abs=1e-9)


def test_legacy_interface_uses_secondary_masses_override():
    """Daughter masses flow from the model's SecondaryMasses override through
    the sampler bridge into the finalized record (not from the mass map)."""
    inj = _legacy_decay_injector(OverrideMassDecay())
    (tree,) = inj.generate(1, on_shortfall="raise")
    (datum,) = list(tree.tree)
    rec = datum.record
    assert list(rec.secondary_masses) == pytest.approx([0.004, 0.002])
    for m, p in zip([0.004, 0.002], rec.secondary_momenta):
        m2 = p[0] ** 2 - p[1] ** 2 - p[2] ** 2 - p[3] ** 2
        assert m2 == pytest.approx(m * m, abs=1e-12)
...
class _MissingTotalWidthDecay(siren.DecayModel):
    """Never overrides total_width(); the authoring-base default only raises
    NotImplementedError when the engine actually calls it, so audit_overrides
    must catch the omission earlier, at Injector build time."""
...
def test_audit_flags_decay_missing_total_width():
    from siren import _validation
    with pytest.raises(ConfigurationError):
        _validation.audit_overrides([_MissingTotalWidthDecay()])
