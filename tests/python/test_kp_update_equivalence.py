...
The Kleiss-Pittau accumulation and weight update run in C++.  These tests
pin that the C++ update reproduces the reference Python `_kp_update`
bit-for-bit, and that nested inner/outer point counts stay in lockstep so
each level divides its accumulator by the same count.
"""
import math
