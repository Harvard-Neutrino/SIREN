...
class TestClosureTests:
    ...
    def test_multichannel_effective_sample_size(self):
        ...
        mc = siren.injection.MultiChannelPhaseSpace()
        mc.channels = [iso, directed]
        # 5% on the isotropic fallback (not 1%): with only 1% the ~25 iso-drawn
        # events that miss the target carry w~100 and dominate sum(w^2), so the
        # ESS efficiency this test asserts requires a non-starved fallback.
        mc.weights = [0.05, 0.95]
...
class TestTopologyMeasureValidation:
    ...
    def test_channel_reports_correct_topology_and_measure(self):
        ...
        assert sc_q2.Measure() == siren.injection.PhaseSpaceMeasure.MandelstamQ2Phi()
        ...
        assert sc_by.Measure() == siren.injection.PhaseSpaceMeasure.FixedMassYPhi()

        assert siren.injection.PhaseSpaceDensityConvertible(
            siren.injection.PhaseSpaceTopology.Scatter2to2,
            siren.injection.PhaseSpaceMeasure.FixedMassY(),
            siren.injection.PhaseSpaceMeasure.FixedMassYPhi(),
        )
        assert not siren.injection.PhaseSpaceDensityConvertible(
            siren.injection.PhaseSpaceTopology.Scatter2to2,
            siren.injection.PhaseSpaceMeasure.FixedMassYPhi(),
            siren.injection.PhaseSpaceMeasure.FixedMassY(),
        )
...
# ================================================================== #
#  Directed channel boundary tests                          #
# ================================================================== #
...
# ================================================================== #
#  Normalization with tighter tolerance                     #
# ================================================================== #
...
class _MockIsotropic2BodyDecay(siren.interactions.Decay):
    ...
    def Measure(self):
        return siren.injection.PhaseSpaceMeasure.SolidAngleRest()
...
class _MockElasticScattering(siren.interactions.CrossSection):
    ...
    def Measure(self):
        return siren.injection.PhaseSpaceMeasure.MandelstamQ2()


# ================================================================== #
#  Cross-measure conversion in multi-channel                    #
# ================================================================== #


class TestCrossMeasureConversion:

    def test_physical_adapter_accepts_explicit_convention_override(self):
        xs = _MockElasticScattering(0.008, 37.215)
        sig = xs.GetPossibleSignatures()[0]
        convention = siren.injection.PhaseSpaceConvention(
            siren.injection.PhaseSpaceTopology.Scatter2to2,
            siren.injection.PhaseSpaceMeasure.MandelstamQ2Phi(),
        )

        physical = siren.injection.PhysicalCrossSectionChannel(
            xs, sig, convention)
        assert physical.Topology() == convention.topology
        assert physical.Measure() == convention.measure
...
# ================================================================== #
#  Scattering channel density tests                             #
# ================================================================== #
...
class TestScatteringChannelDensity:
    ...
    def test_physical_scattering_with_directed_multichannel(self):
        """Combine an azimuth-integrated physical Q2 density with a
        detector-directed Q2,Phi proposal. The mixture lifts the physical
        density with its uniform 1/(2*pi) azimuthal conditional.
        ...
        assert directed.Measure() == siren.injection.PhaseSpaceMeasure.MandelstamQ2Phi()
        ...
        for _ in range(N):
            r = copy.deepcopy(rec)
            mc.Sample(rng, None, r)
            d_phys = siren.injection.ConvertDensity(
                phys.Density(None, r), phys.Measure(), mc.CommonMeasure(),
                mc.CommonTopology(), r)
            d_mc = mc.Density(None, r)
            if d_mc > 0:
                weights.append(d_phys / d_mc)
