"""Generation-time accumulation pieces. The chain optimizer feeds the
mixtures' accumulators in C++ rather than walking trees in Python.  These
tests pin the two genuinely-new C++ pieces:
...
def test_updateweights_failure_penalty_inflates_W():
    ...
    # explicit "coverage": this test pins the coverage inflation mode directly,
    # rather than relying on the "throughput" default.
...
def test_injector_routing_event_credits_all_datums_selection_once():
    ...
    assert [tree.depth(i) for i in range(len(tree.tree))] == [0, 0]
