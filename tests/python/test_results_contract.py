...
# --------------------------------------------------------------------------- #
# Real-path snapshot: requested is the request, not the retry budget            #
# --------------------------------------------------------------------------- #

def test_run_requested_is_the_request_not_the_retry_budget():
    """A real Simulation.run(events=N) reports requested == N, not the internal retry budget (events * 1000) that generate() uses. Exercises the real generate/weight path end to end."""
    N = 3
    ...
    # requested is the request (N), not the retry budget (1000 * N).
    assert results.requested == N
    assert len(results) == N
    # The snapshot's run-level counts stay internally consistent.
    assert results.injected <= results.attempts
