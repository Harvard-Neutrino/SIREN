...
cat dDipole_input.txt | while read D; do
		OUTPUT="Outputs_MINERvA_Bessel/ME_${beam}_${primary}_mHNL_${M}_d_${D}"
		...
		./inject_dipole_MINERvA --output $OUTPUT --li $LI_PATH --earth-model $EARTH_MODEL --materials-model $MAT_MODEL --tot-xsec-path $TOT_XSEC_PATH --diff-xsec-path $DIF_XSEC_PATH -z $Z -n $N -m $M -d $D --${beam} --ME --${primary} --seed $SEED --flux-file $FLUX_FILE --sparse 1 
done
done
