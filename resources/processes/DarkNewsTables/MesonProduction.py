...
The generic three-body machinery (the Dalitz band and width integral,
the |M|^2 rejection sampler, rest-frame momentum construction, boosts,
and the Recursive2Body density) comes from the BeamDecays process
resource; this module supplies the matrix elements, the lab-frame V1
marginals, and the cone-biased sampler. build_phi_flux() constructs
the scalar mediator flux at the detector. The SM two-body decay
M -> mu nu lives in BeamDecays as MesonTwoBodyLeptonicDecay.
...
# Shared three-body Dalitz machinery from the BeamDecays resource. The
# physics objects in this module satisfy its duck-type contract: m_M,
# m_l, m_phi, E_nu_max, _matel_sq(E_nu, E_phi), _E_phi_limits(E_nu).
_beam = _siren_util.load_module(
    "siren_resources_beam_decays",
    os.path.normpath(os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        os.pardir, "BeamDecays", "processes.py")),
)
_dalitz_band = _beam._dalitz_band
_dalitz_width = _beam._dalitz_width
_find_max_weight = _beam._find_max_weight
_sample_energies = _beam._sample_energies
_build_rest_momenta = _beam._build_rest_momenta
_boost_four_vector = _beam._boost_four_vector
_final_state_probability = _beam._final_state_probability
...
class MesonThreeBodyDecay:
    ...
    def total_width(self):
        """Total three-body decay width [GeV] from the shared Dalitz integral."""
...
# ===================================================================
#  Lab-frame V1 marginals (rest-frame <-> lab-frame kinematics)
# ===================================================================
...
class MesonThreeBodySIRENDecay(_Decay):
    ...
    def __init__(
        ...
        *,
        ...
        self._max_matel = _find_max_weight(self._decay)
    ...
    def FinalStateProbability(self, record):
        ...
        """
        return _final_state_probability(
    ...
    def MeasureForSignature(self, signature):
        # Single-signature model: the per-signature measure is Measure().
        return self.Measure()
    ...
    def SampleFinalState(self, record, random):
        ...
        E_nu_rf, E_phi_rf = _sample_energies(self._decay, self._max_matel, random)
        P_parent = np.array(record.primary_momentum)
        p_nu, p_l, p_phi = _build_rest_momenta(
            self._decay, E_nu_rf, E_phi_rf, random)

        for sec in record.get_secondary_particle_records():
            pid = int(sec.type)
            if pid == self.pdgid_lepton:
                sec.four_momentum = _boost_four_vector(
                    P_parent, p_l, self._decay.m_M)
                sec.mass = self._decay.m_l
            elif pid == self.pdgid_neutrino:
                sec.four_momentum = _boost_four_vector(
                    P_parent, p_nu, self._decay.m_M)
                sec.mass = 0.0
            elif pid == self.pdgid_mediator:
                sec.four_momentum = _boost_four_vector(
                    P_parent, p_phi, self._decay.m_M)
