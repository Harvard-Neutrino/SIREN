...
std::shared_ptr<SI::geometry::Geometry> DetectorModel::ParseGeometryObject(std::stringstream & ss) {
    std::string shape;
...
std::shared_ptr<SI::detector::DensityDistribution> DetectorModel::ParseDensityDistribution(std::stringstream & ss) {
...
std::tuple<SI::math::Vector3D, SI::math::Quaternion> DetectorModel::ParseDetector(std::stringstream & ss) {
    ...
    SI::math::Vector3D detector_origin(x0, y0, z0);

    SI::math::Quaternion detector_rotation;
    try {
        double alpha, beta, gamma; // Euler Angles of shape rotation
        ss >> alpha >> beta >> gamma;
        detector_rotation = QFromZXZr(alpha, beta, gamma);
    } catch(std::ios_base::failure e) {
        detector_rotation = SI::math::Quaternion();
    }
    return {detector_origin, detector_rotation};
}

std::shared_ptr<SI::geometry::Geometry> DetectorModel::ParseFiducialVolume(std::string fiducial_line, std::string origin_line) {
    std::string line = origin_line;
    std::stringstream ss(line);

    std::tuple<SI::math::Vector3D, SI::math::Quaternion> detector = ParseDetector(ss);

    return ParseFiducialVolume(fiducial_line, std::get<0>(detector), std::get<1>(detector));
}

std::shared_ptr<SI::geometry::Geometry> DetectorModel::ParseFiducialVolume(std::string fiducial_line, SI::math::Vector3D detector_origin, SI::math::Quaternion detector_quaternion) {
    ...
    std::shared_ptr<SI::geometry::Geometry> geo = ParseGeometryObject(ss);
...
double DetectorModel::GetParticleDensity(Geometry::IntersectionList const & intersections, GeometryPosition const & p0, SI::dataclasses::ParticleType target) const {
    Vector3D direction = p0 - intersections.position;
    if(direction.magnitude() == 0) {
        direction = intersections.direction;
    } else {
...
double DetectorModel::GetParticleDensity(GeometryPosition const & p0, SI::dataclasses::ParticleType target) const {
    Vector3D direction(1,0,0); // Any direction will work for determining the sector heirarchy
    Geometry::IntersectionList intersections = GetIntersections(p0, GeometryDirection(direction));
...
double DetectorModel::GetMassDensity(Geometry::IntersectionList const & intersections, GeometryPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    Vector3D direction = p0 - intersections.position;
    if(direction.magnitude() == 0) {
        direction = intersections.direction;
    } else {
...
double DetectorModel::GetMassDensity(GeometryPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    Vector3D direction(1,0,0); // Any direction will work for determining the sector heirarchy
    Geometry::IntersectionList intersections = GetIntersections(p0, GeometryDirection(direction));
    return GetMassDensity(intersections, p0, targets);
}

std::vector<double> DetectorModel::GetParticleDensity(Geometry::IntersectionList const & intersections, GeometryPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    Vector3D direction = p0 - intersections.position;
    if(direction.magnitude() == 0) {
        direction = intersections.direction;
    } else {
...
std::vector<double> DetectorModel::GetParticleDensity(GeometryPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    Vector3D direction(1,0,0); // Any direction will work for determining the sector heirarchy
    Geometry::IntersectionList intersections = GetIntersections(p0, GeometryDirection(direction));
...
std::vector<double> DetectorModel::GetParticleColumnDepth(Geometry::IntersectionList const & intersections, GeometryPosition const & p0, GeometryPosition const & p1,  std::vector<SI::dataclasses::ParticleType> const & targets) const {
    if(p0 == p1) {
...
std::set<SI::dataclasses::ParticleType> DetectorModel::GetAvailableTargets(GeometryPosition const & vertex) const {
...
std::set<SI::dataclasses::ParticleType> DetectorModel::GetAvailableTargets(geometry::Geometry::IntersectionList const & intersections, GeometryPosition const & vertex) const {
    int matID = GetContainingSector(intersections, vertex).material_id;
    std::vector<SI::dataclasses::ParticleType> particles = materials_.GetMaterialConstituents(matID);
    return std::set<SI::dataclasses::ParticleType>(particles.begin(), particles.end());
}
...
double DetectorModel::DistanceForInteractionDepthFromPoint(Geometry::IntersectionList const & intersections, GeometryPosition const & p0, GeometryDirection const & dir, double interaction_depth,
    ...
    double total_decay_length_cm = total_decay_length / SI::utilities::Constants::cm;
    ...
    std::function<bool(std::vector<Geometry::Intersection>::const_iterator, std::vector<Geometry::Intersection>::const_iterator, double)> callback =
        ...
        if(end_point > 0) {
            ...
            integral *= (target_composition*SI::utilities::Constants::m/SI::utilities::Constants::cm); // --> m cm^-1 --> dimensionless
...
double DetectorModel::GetParticleDensity(Geometry::IntersectionList const & intersections, DetectorPosition const & p0, SI::dataclasses::ParticleType target) const {
    return GetParticleDensity(intersections, ToGeo(p0), target);
}

double DetectorModel::GetParticleDensity(DetectorPosition const & p0, SI::dataclasses::ParticleType target) const {
...
double DetectorModel::GetMassDensity(Geometry::IntersectionList const & intersections, DetectorPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    return GetMassDensity(intersections, ToGeo(p0), targets);
}

double DetectorModel::GetMassDensity(DetectorPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    return GetMassDensity(ToGeo(p0), targets);
}

std::vector<double> DetectorModel::GetParticleDensity(Geometry::IntersectionList const & intersections, DetectorPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
    return GetParticleDensity(intersections, ToGeo(p0), targets);
}

std::vector<double> DetectorModel::GetParticleDensity(DetectorPosition const & p0,  std::set<SI::dataclasses::ParticleType> targets) const {
...
std::vector<double> DetectorModel::GetParticleColumnDepth(Geometry::IntersectionList const & intersections, DetectorPosition const & p0, DetectorPosition const & p1,  std::vector<SI::dataclasses::ParticleType> const & targets) const {
...
std::set<SI::dataclasses::ParticleType> DetectorModel::GetAvailableTargets(DetectorPosition const & vertex) const {
    return GetAvailableTargets(ToGeo(vertex));
}

std::set<SI::dataclasses::ParticleType> DetectorModel::GetAvailableTargets(geometry::Geometry::IntersectionList const & intersections, DetectorPosition const & vertex) const {
...
void DetectorModel::LoadConcentricShellsFromLegacyFile(std::string model_fname, double detector_depth, double ice_cap_angle) {
    ...
    if(ice_cap_angle < 0 || ice_cap_angle > SI::utilities::Constants::pi) {
        // Leave any ice as is
    }
...
double DetectorModel::GetTargetMass(SI::dataclasses::ParticleType target) const {
    double molar_mass = materials_.GetMolarMass(target); // grams per mole
    return molar_mass * SI::utilities::Constants::GeV_per_amu;
}
