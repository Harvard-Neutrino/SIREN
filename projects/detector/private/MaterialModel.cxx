...
MaterialModel::Component::Component(SI::dataclasses::ParticleType type)
    : type(type)
{
    if(type == SI::dataclasses::ParticleType::PPlus) {
        neutron_count = 0;
        proton_count = 1;
        nucleon_count = 1;
        is_atom = false;
        molar_mass = atomic_masses.at({strange_count, neutron_count, proton_count, nucleon_count});
    } else if (type == SI::dataclasses::ParticleType::Neutron) {
        neutron_count = 1;
        proton_count = 0;
        nucleon_count = 1;
        is_atom = false;
        molar_mass = atomic_masses.at({strange_count, neutron_count, proton_count, nucleon_count});
    } else if (type == SI::dataclasses::ParticleType::Nucleon) {
        neutron_count = 0;
        proton_count = 0;
        nucleon_count = 1;
        is_atom = false;
        molar_mass = (atomic_masses.at({0, 1, 0, 1}) + atomic_masses.at({0, 0, 1, 1})) / 2.0;
    } else if (type == SI::dataclasses::ParticleType::EMinus) {
        neutron_count = 0;
        proton_count = 0;
        nucleon_count = 0;
        is_atom = false;
        ...
        try {
            ...
            if(it != atomic_masses.end())
                ...
                molar_mass = strange_count * SI::utilities::Constants::lambda0Mass / SI::utilities::Constants::GeV_per_amu
                    + neutron_count * atomic_masses.at({0, 1, 0, 1})
                    + proton_count * atomic_masses.at({0, 0, 1, 1})
                    - binding_energy / SI::utilities::Constants::GeV_per_amu;
            }
...
void MaterialModel::AddMaterial(std::string const & material_name, std::map<int, double> const & component_mass_fractions) {
    ...
    for(auto const & mass_frac_it : component_mass_fractions) {
        ...
        Component component(static_cast<SI::dataclasses::ParticleType>(component_id));
        ...
        material_component.particle_density_over_total_mass_density = SI::utilities::Constants::avogadro * component_mass_fraction / material_component.component.molar_mass;
    ...
    if(neutrons_per_amu > 0) {
        Component component(SI::dataclasses::ParticleType::Neutron);
        MaterialComponent material_component;
        material_component.component = component;
        material_component.mass_density_over_total_mass_density = neutrons_per_amu * component.molar_mass;
        material_component.particle_density_over_total_mass_density = neutrons_per_amu * SI::utilities::Constants::avogadro;
        material_components.push_back(material_component);
    }

    if(nucleons_per_amu > 0) {
        Component component(SI::dataclasses::ParticleType::Nucleon);
        MaterialComponent material_component;
        material_component.component = component;
        material_component.mass_density_over_total_mass_density = nucleons_per_amu * component.molar_mass;
        material_component.particle_density_over_total_mass_density = nucleons_per_amu * SI::utilities::Constants::avogadro;
        material_components.push_back(material_component);
    }

    if(protons_per_amu > 0) {
        Component component(SI::dataclasses::ParticleType::PPlus);
        MaterialComponent material_component;
        material_component.component = component;
        material_component.mass_density_over_total_mass_density = protons_per_amu * component.molar_mass;
        material_component.particle_density_over_total_mass_density = protons_per_amu * SI::utilities::Constants::avogadro;
        material_components.push_back(material_component);
        component = Component(SI::dataclasses::ParticleType::EMinus);
        material_component.component = component;
        material_component.mass_density_over_total_mass_density = protons_per_amu * component.molar_mass;
        material_component.particle_density_over_total_mass_density = protons_per_amu * SI::utilities::Constants::avogadro;
        material_components.push_back(material_component);
...
std::vector<SI::dataclasses::ParticleType> MaterialModel::GetMaterialTargets(int material_id) const {
...
double MaterialModel::GetTargetMassFraction(int material_id, SI::dataclasses::ParticleType particle_type) const {
    std::pair<int, SI::dataclasses::ParticleType> key(material_id, particle_type);
...
double MaterialModel::GetTargetParticleFraction(int material_id, SI::dataclasses::ParticleType particle_type) const {
    std::pair<int, SI::dataclasses::ParticleType> key(material_id, particle_type);
    if(material_components_by_id_.find(key) != material_components_by_id_.end())
...
std::vector<double> MaterialModel::GetTargetMassFraction(int material_id, std::vector<SI::dataclasses::ParticleType> const & particle_types) const {
...
std::vector<double> MaterialModel::GetTargetParticleFraction(int material_id, std::vector<SI::dataclasses::ParticleType> const & particle_types) const {
    std::vector<double> fractions;
    fractions.reserve(particle_types.size());
    for(auto const & particle_type : particle_types) {
        std::pair<int, SI::dataclasses::ParticleType> key(material_id, particle_type);
...
std::vector<double> MaterialModel::GetTargetRadiationFraction(int material_id, std::vector<SI::dataclasses::ParticleType> const & particle_types) const {
    double X0inv = 0;
    std::vector<double> fractions;
    fractions.reserve(particle_types.size());
    for(auto const & particle_type : particle_types) {
...
std::vector<SI::dataclasses::ParticleType> MaterialModel::GetMaterialConstituents(int material_id) const {
    std::vector<SI::dataclasses::ParticleType> particles;;
...
double MaterialModel::GetMolarMass(SI::dataclasses::ParticleType particle) {
    return Component(particle).molar_mass;
}

int MaterialModel::GetStrangeCount(SI::dataclasses::ParticleType particle) {
    return Component(particle).strange_count;
}

int MaterialModel::GetNucleonCount(SI::dataclasses::ParticleType particle) {
    return Component(particle).nucleon_count;
}

int MaterialModel::GetNeutronCount(SI::dataclasses::ParticleType particle) {
    return Component(particle).neutron_count;
}

int MaterialModel::GetProtonCount(SI::dataclasses::ParticleType particle) {
...
int MaterialModel::GetEmpericalNuclearBindingEnergy(int strange_count, int neutron_count, int proton_count, int nucleon_count) {
    ...
    const double lambda0_mass = SI::utilities::Constants::lambda0Mass * 1e3; // GeV --> MeV
