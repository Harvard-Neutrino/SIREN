...
// Returns true if the ray (origin, direction) intersects the AABB.
// tmin and tmax are set to the entry and exit distances along the ray.
// inv_direction should be precomputed as 1.0/direction for each component.
// Uses std::fmin/std::fmax for NaN safety: when a direction component is zero,
// inv_direction is +/-inf, and (boundary - origin) * inf can produce NaN
// if origin is exactly on the boundary. fmin/fmax propagate the non-NaN value.
inline bool RayAABBIntersect(
    ...
    double & tmin,
    double & tmax) {
    double t1 = (box.min_corner.GetX() - origin.GetX()) * inv_direction.GetX();
    double t2 = (box.max_corner.GetX() - origin.GetX()) * inv_direction.GetX();

    tmin = std::fmin(t1, t2);
    tmax = std::fmax(t1, t2);

    t1 = (box.min_corner.GetY() - origin.GetY()) * inv_direction.GetY();
    t2 = (box.max_corner.GetY() - origin.GetY()) * inv_direction.GetY();

    tmin = std::fmax(tmin, std::fmin(t1, t2));
    tmax = std::fmin(tmax, std::fmax(t1, t2));

    t1 = (box.min_corner.GetZ() - origin.GetZ()) * inv_direction.GetZ();
    t2 = (box.max_corner.GetZ() - origin.GetZ()) * inv_direction.GetZ();

    tmin = std::fmax(tmin, std::fmin(t1, t2));
    tmax = std::fmin(tmax, std::fmax(t1, t2));

    // Check tmax >= tmin for intersection.
