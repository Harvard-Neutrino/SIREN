...
Cone::Cone() : Geometry("Cone"), rmin1_(0), rmax1_(0), rmin2_(0), rmax2_(0), z_(0), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) { RecomputeWorldAABB(); }
Cone::Cone(double rmin1, double rmax1, double rmin2, double rmax2, double z) : Geometry("Cone"), rmin1_(rmin1), rmax1_(rmax1), rmin2_(rmin2), rmax2_(rmax2), z_(z), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) {
    if(z_ <= 0) throw std::invalid_argument("Cone height must be positive!");
    if(rmin1_ > rmax1_) std::swap(rmin1_, rmax1_);
    if(rmin2_ > rmax2_) std::swap(rmin2_, rmax2_);
    if(rmax1_ <= 0 && rmax2_ <= 0) throw std::invalid_argument("Cone must have at least one positive outer radius!");
    RecomputeWorldAABB();
}
Cone::Cone(double rmin1, double rmax1, double rmin2, double rmax2, double z, double start_phi, double delta_phi) : Geometry("Cone"), rmin1_(rmin1), rmax1_(rmax1), rmin2_(rmin2), rmax2_(rmax2), z_(z), start_phi_(start_phi), delta_phi_(delta_phi) {
    if(z_ <= 0) throw std::invalid_argument("Cone height must be positive!");
    if(rmin1_ > rmax1_) std::swap(rmin1_, rmax1_);
    if(rmin2_ > rmax2_) std::swap(rmin2_, rmax2_);
    if(rmax1_ <= 0 && rmax2_ <= 0) throw std::invalid_argument("Cone must have at least one positive outer radius!");
...
Cone::Cone(Placement const & p) : Geometry("Cone", p), rmin1_(0), rmax1_(0), rmin2_(0), rmax2_(0), z_(0), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) { RecomputeWorldAABB(); }
Cone::Cone(Placement const & p, double rmin1, double rmax1, double rmin2, double rmax2, double z) : Geometry("Cone", p), rmin1_(rmin1), rmax1_(rmax1), rmin2_(rmin2), rmax2_(rmax2), z_(z), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) {
    if(z_ <= 0) throw std::invalid_argument("Cone height must be positive!");
    if(rmin1_ > rmax1_) std::swap(rmin1_, rmax1_);
    if(rmin2_ > rmax2_) std::swap(rmin2_, rmax2_);
    if(rmax1_ <= 0 && rmax2_ <= 0) throw std::invalid_argument("Cone must have at least one positive outer radius!");
...
Cone::Cone(Placement const & p, double rmin1, double rmax1, double rmin2, double rmax2, double z, double start_phi, double delta_phi) : Geometry("Cone", p), rmin1_(rmin1), rmax1_(rmax1), rmin2_(rmin2), rmax2_(rmax2), z_(z), start_phi_(start_phi), delta_phi_(delta_phi) {
    if(z_ <= 0) throw std::invalid_argument("Cone height must be positive!");
    if(rmin1_ > rmax1_) std::swap(rmin1_, rmax1_);
    if(rmin2_ > rmax2_) std::swap(rmin2_, rmax2_);
    if(rmax1_ <= 0 && rmax2_ <= 0) throw std::invalid_argument("Cone must have at least one positive outer radius!");
...
Cone::Cone(const Cone& o) : Geometry(o), rmin1_(o.rmin1_), rmax1_(o.rmax1_), rmin2_(o.rmin2_), rmax2_(o.rmax2_), z_(o.z_), start_phi_(o.start_phi_), delta_phi_(o.delta_phi_), has_phi_cut_(o.has_phi_cut_) { RecomputeWorldAABB(); }

SIREN_GEOMETRY_SWAP(Cone, rmin1_, rmax1_, rmin2_, rmax2_, z_, start_phi_, delta_phi_, has_phi_cut_)
SIREN_GEOMETRY_ASSIGN(Cone)
SIREN_GEOMETRY_EQUAL(Cone, rmin1_, rmax1_, rmin2_, rmax2_, z_, start_phi_, delta_phi_, has_phi_cut_)
SIREN_GEOMETRY_LESS(Cone, rmin1_, rmax1_, rmin2_, rmax2_, z_, start_phi_, delta_phi_, has_phi_cut_)

void Cone::print(std::ostream& os) const {
    os << "Cone(" << rmin1_ << ", " << rmax1_ << ", " << rmin2_ << ", " << rmax2_ << ", " << z_;
...
std::vector<Geometry::Intersection> Cone::ComputeIntersections(siren::math::Vector3D const & position, siren::math::Vector3D const & direction) const {
    ...
    static constexpr int HIT_CAP = 12;
    TaggedHit all_hits[HIT_CAP]; // max: 2 outer + 2 inner + 2 top + 2 bottom + 2 wedge + spare
    int n_all = 0;
