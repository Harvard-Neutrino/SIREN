...
// ---- Phi-cut variants (has_phi_cut_ recomputed on deserialization) ----

TEST(SerializationRoundtrip, CylinderPhiCut) {
    auto cyl = std::make_shared<Cylinder>(5.0, 2.0, 10.0, 0.0, M_PI);
    auto loaded = RoundtripGeometry(cyl);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*cyl, *loaded);
    EXPECT_EQ(CountIntersections(*cyl, Vector3D(3, 0, -20), Vector3D(0, 0, 1)),
              CountIntersections(*loaded, Vector3D(3, 0, -20), Vector3D(0, 0, 1)));
}

TEST(SerializationRoundtrip, ConePhiCut) {
    auto cone = std::make_shared<Cone>(0.0, 5.0, 0.0, 3.0, 8.0, 0.0, M_PI);
    auto loaded = RoundtripGeometry(cone);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*cone, *loaded);
}

TEST(SerializationRoundtrip, PolyconePhiCut) {
    auto pc = std::make_shared<Polycone>(
        std::vector<double>{-5, 0, 5},
        std::vector<double>{0, 0, 0},
        std::vector<double>{3, 5, 3},
        0.0, M_PI);
    auto loaded = RoundtripGeometry(pc);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*pc, *loaded);
}

TEST(SerializationRoundtrip, CutTubePhiCut) {
    auto ct = std::make_shared<CutTube>(1.0, 5.0, 8.0,
        Vector3D(0, 0, -1), Vector3D(0, 0, 1), 0.0, M_PI);
    auto loaded = RoundtripGeometry(ct);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*ct, *loaded);
}

TEST(SerializationRoundtrip, GenericPolyconePhiCut) {
    auto gpc = std::make_shared<GenericPolycone>(
        std::vector<double>{0, 5, 0},
        std::vector<double>{-5, 0, 5},
        0.0, M_PI);
    auto loaded = RoundtripGeometry(gpc);
    ASSERT_NE(loaded, nullptr);
...
TEST(SerializationRoundtrip, PolyhedraPhiCut) {
    auto ph = std::make_shared<Polyhedra>(
        6, 0.0,
        std::vector<double>{-4.0, 4.0},
        std::vector<double>{0.0, 0.0},
        std::vector<double>{5.0, 5.0},
        M_PI);
    auto loaded = RoundtripGeometry(ph);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*ph, *loaded);
    EXPECT_EQ(CountIntersections(*ph, Vector3D(0, 3, -10), Vector3D(0, 0, 1)),
              CountIntersections(*loaded, Vector3D(0, 3, -10), Vector3D(0, 0, 1)));
}

TEST(SerializationRoundtrip, ExtrPoly) {
    std::vector<std::vector<double>> polygon = {{-3,-3},{3,-3},{3,3},{-3,3}};
    double off[2] = {0, 0};
    std::vector<ExtrPoly::ZSection> zsecs = {
        ExtrPoly::ZSection(-4, off, 1.0),
        ExtrPoly::ZSection(4, off, 0.5)
    };
    auto ep = std::make_shared<ExtrPoly>(polygon, zsecs);
    auto loaded = RoundtripGeometry(ep);
    ASSERT_NE(loaded, nullptr);
    EXPECT_EQ(*ep, *loaded);
    EXPECT_TRUE(loaded->IsInside(Vector3D(0, 0, 0)));
    EXPECT_FALSE(loaded->IsInside(Vector3D(10, 0, 0)));
}
