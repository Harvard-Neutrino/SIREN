...
bool InsideRegularPolygon(double px, double py, int n, double r_apothem, double start_phi) {
    // r_apothem is the tangent distance (center to face midpoint).
    // Vertex distance (circumradius) = r_apothem / cos(pi/n).
    double r = r_apothem / std::cos(M_PI / n);
...
bool InsideTrap(Vector3D const & p, double dz, double theta, double phi,
    ...
    for(int f = 0; f < 6; ++f) {
        ...
        // Strictly inside: point must be on the inner side of every face
...
bool InsidePara(Vector3D const & p, double dx, double dy, double dz,
    ...
    // G4Para convention: z spans [-dz, +dz] flat, face centers offset by tan(theta)
    double w = p.GetZ() / dz;
    if(std::fabs(w) >= 1.0) return false;
    double e3x = std::tan(theta) * std::cos(phi);
    double e3y = std::tan(theta) * std::sin(phi);
...
TEST(Containment, BoxRotated) {
    double x = 10, y = 8, z = 6;
    Quaternion q(std::cos(0.3), std::sin(0.3)*INV_SQRT3, std::sin(0.3)*INV_SQRT3, std::sin(0.3)*INV_SQRT3);
...
TEST(Containment, ConeRotated) {
    ...
    Quaternion q(std::cos(0.4), std::sin(0.4)*INV_SQRT3, std::sin(0.4)*INV_SQRT3, std::sin(0.4)*INV_SQRT3);
...
TEST(Containment, PolyconeRotated) {
    ...
    Quaternion q(std::cos(0.5), std::sin(0.5)*INV_SQRT3, std::sin(0.5)*INV_SQRT3, std::sin(0.5)*INV_SQRT3);
...
TEST(Containment, PolyhedraRotated) {
    ...
    Quaternion q(std::cos(0.6), std::sin(0.6)*INV_SQRT3, std::sin(0.6)*INV_SQRT3, std::sin(0.6)*INV_SQRT3);
...
// =========================================================================
// BooleanGeometry with ray from inside
// =========================================================================
...
// =========================================================================
// Polycone with step-change in radius
// =========================================================================
...
// =========================================================================
// ExtrPoly with non-zero offset in z-sections
// =========================================================================
...
// =========================================================================
// =========================================================================
// Containment at Polycone/Polyhedra internal z-boundaries
...
TEST(Containment, CylinderHollowPhiHalf) {
    double rmax = 5, rmin = 2, z = 12;
    double sp = 0, dp = M_PI;
    Cylinder cyl(rmax, rmin, z, sp, dp);
    ValidateContainment(cyl, [=](Vector3D const & p) {
        return InsideCylinder(p, rmax, rmin, z) && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 10, 10000, "CylinderHollowPhiHalf");
}

TEST(Containment, ConeHollowPhiHalf) {
    double rmin1 = 1, rmax1 = 5, rmin2 = 1, rmax2 = 3, z = 8;
    double sp = 0, dp = M_PI;
    Cone cone(rmin1, rmax1, rmin2, rmax2, z, sp, dp);
    ValidateContainment(cone, [=](Vector3D const & p) {
        return InsideCone(p, rmin1, rmax1, rmin2, rmax2, z) && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 12, 10000, "ConeHollowPhiHalf");
}

TEST(Containment, CutTubePhiHalf) {
    double rmin = 0, rmax = 5, dz = 8;
    Vector3D low(0, 0, -1), high(0, 0, 1);
    double sp = 0, dp = M_PI;
    CutTube ct(rmin, rmax, dz, low, high, sp, dp);
    ValidateContainment(ct, [=](Vector3D const & p) {
        return InsideCutTube(p, rmin, rmax, dz, low, high) && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 10, 10000, "CutTubePhiHalf");
}

TEST(Containment, CutTubeHollowPhiHalf) {
    double rmin = 2, rmax = 5, dz = 8;
    Vector3D low(0, 0, -1), high(0, 0, 1);
    double sp = 0, dp = M_PI;
    CutTube ct(rmin, rmax, dz, low, high, sp, dp);
    ValidateContainment(ct, [=](Vector3D const & p) {
        return InsideCutTube(p, rmin, rmax, dz, low, high) && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 10, 10000, "CutTubeHollowPhiHalf");
}

TEST(Containment, PolyconeHollowPhiHalf) {
    std::vector<double> zp = {-4, 0, 4};
    std::vector<double> rmin_v = {1, 2, 1};
    std::vector<double> rmax_v = {5, 6, 5};
    double sp = 0, dp = M_PI;
    Polycone pc(zp, rmin_v, rmax_v, sp, dp);
    ValidateContainment(pc, [&](Vector3D const & p) {
        return InsidePolycone(p, zp, rmin_v, rmax_v) && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 10, 10000, "PolyconeHollowPhiHalf");
}

TEST(Containment, TorusHollowPhiHalf) {
    double R = 10, r = 3, ri = 1;
    double sp = 0, dp = M_PI;
    Torus torus(R, r, ri, sp, dp);
    ValidateContainment(torus, [=](Vector3D const & p) {
        return InsideTorusPartial(p, R, r, ri, sp, dp);
    }, 15, 10000, "TorusHollowPhiHalf");
}

TEST(Containment, GenericPolyconePhiHalf) {
    double sp = 0, dp = M_PI;
    GenericPolycone gpc({0, 5, 0}, {-5, 0, 5}, sp, dp);
    ValidateContainment(gpc, [=](Vector3D const & p) {
        double rxy = std::sqrt(p.GetX()*p.GetX() + p.GetY()*p.GetY());
        double pz = p.GetZ();
        bool in_bicone = false;
        if(pz > -5 && pz < 0) { in_bicone = rxy < 5.0 * (pz + 5) / 5.0; }
        else if(pz >= 0 && pz < 5) { in_bicone = rxy < 5.0 * (5 - pz) / 5.0; }
        return in_bicone && InsidePhiRange(p.GetX(), p.GetY(), sp, dp);
    }, 8, 10000, "GenericPolyconePhiHalf");
}
...
TEST(Containment, Polyhedra6Phi180) {
    int n = 6;
    double sp = 0;
    double dp = M_PI;
    Polyhedra ph(n, sp, {-4, 4}, {0, 0}, {5, 5}, dp);
    double side_angle = dp / n;
    ValidateContainment(ph, [=](Vector3D const & p) {
        double px = p.GetX(), py = p.GetY(), pz = p.GetZ();
        if(pz <= -4 || pz >= 4) return false;
        double vertex_r = 5.0 / std::cos(0.5 * side_angle);
        for(int k = 0; k < n; ++k) {
            double a0 = sp + k * side_angle;
            double a1 = sp + (k + 1) * side_angle;
            double vx0 = vertex_r*std::cos(a0), vy0 = vertex_r*std::sin(a0);
            double vx1 = vertex_r*std::cos(a1), vy1 = vertex_r*std::sin(a1);
            double ex = vx1-vx0, ey = vy1-vy0;
            if(ey*(px-vx0) - ex*(py-vy0) > 0) return false;
        }
        return InsidePhiRange(px, py, sp, dp);
    }, 8, 10000, "Polyhedra6Phi180");
}

// =========================================================================
// Additional sphere theta-cut variants
// =========================================================================

TEST(Containment, SphereThetaCone45) {
    double r = 5, st = 0, dt = M_PI/4;
    Sphere sphere(r, 0, 0, 2*M_PI, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r, 0, 0, 2*M_PI, st, dt);
    }, 8, 10000, "SphereThetaCone45");
}

TEST(Containment, SphereThetaBand45_90) {
    double r = 5, st = M_PI/4, dt = M_PI/4;
    Sphere sphere(r, 0, 0, 2*M_PI, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r, 0, 0, 2*M_PI, st, dt);
    }, 8, 10000, "SphereThetaBand45_90");
}

TEST(Containment, SphereThetaBandMid) {
    double r = 5, st = M_PI/3, dt = M_PI/3;
    Sphere sphere(r, 0, 0, 2*M_PI, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r, 0, 0, 2*M_PI, st, dt);
    }, 8, 10000, "SphereThetaBandMid");
}

TEST(Containment, SphereHollowThetaCone60) {
    double r_out = 5, r_in = 2, st = 0, dt = M_PI/3;
    Sphere sphere(r_out, r_in, 0, 2*M_PI, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r_out, r_in, 0, 2*M_PI, st, dt);
    }, 8, 10000, "SphereHollowThetaCone60");
}

TEST(Containment, SpherePhiThetaQuarter) {
    double r = 5, sp = 0, dp = M_PI/2, st = 0, dt = M_PI/2;
    Sphere sphere(r, 0, sp, dp, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r, 0, sp, dp, st, dt);
    }, 8, 10000, "SpherePhiThetaQuarter");
}

TEST(Containment, SpherePhiHalfThetaBand) {
    double r = 5, sp = 0, dp = M_PI, st = M_PI/4, dt = M_PI/2;
    Sphere sphere(r, 0, sp, dp, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r, 0, sp, dp, st, dt);
    }, 8, 10000, "SpherePhiHalfThetaBand");
}

TEST(Containment, SphereHollowPhiTheta) {
    double r_out = 5, r_in = 2, sp = 0, dp = M_PI, st = 0, dt = M_PI/2;
    Sphere sphere(r_out, r_in, sp, dp, st, dt);
    ValidateContainment(sphere, [=](Vector3D const & p) {
        return InsideSpherePartial(p, r_out, r_in, sp, dp, st, dt);
    }, 8, 10000, "SphereHollowPhiTheta");
}
...
TEST(Containment, TrapGeneral) {
    // Full shear path: theta, phi, alpha all nonzero. Uses equal dx and
    // alpha at both faces so the six faces are planar (true parallelepiped).
...
TEST(Containment, ExtrPolyHexagon) {
    ...
    // Uses explicit vertex coordinates, so the oracle must use the same
    // circumradius directly (NOT InsideRegularPolygon which applies
    // the apothem-to-circumradius scaling for Polyhedra).
    double r = 5.0;
    int n = 6;
    ...
    ExtrPoly ep(polygon, zsecs);
    ValidateContainment(ep, [&](Vector3D const & p) {
        // Edge-normal test directly on the explicit vertices
        double px = p.GetX(), py = p.GetY();
        for(int k = 0; k < n; ++k) {
            int j = (k + 1) % n;
            double ex = vx[j]-vx[k], ey = vy[j]-vy[k];
            if(ey*(px-vx[k]) - ex*(py-vy[k]) > 0) return false;
        }
        return p.GetZ() > -4.0 && p.GetZ() < 4.0;
