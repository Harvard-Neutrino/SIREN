...
CutTube::CutTube() : Geometry("CutTube"), rmin_(0), rmax_(0), dz_(0), low_norm_(math::Vector3D(0, 0, -1)), high_norm_(math::Vector3D(0, 0, 1)), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) { RecomputeWorldAABB(); }
CutTube::CutTube(double rmin, double rmax, double dz, math::Vector3D low_norm, math::Vector3D high_norm) : Geometry("CutTube"), rmin_(rmin), rmax_(rmax), dz_(dz), low_norm_(low_norm), high_norm_(high_norm), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) {
    if(rmin_ < 0) throw std::invalid_argument("CutTube inner radius rmin must be non-negative!");
    if(rmax_ < 0) throw std::invalid_argument("CutTube outer radius rmax must be non-negative!");
    if(rmin_ > rmax_) std::swap(rmin_, rmax_);
    if(rmax_ <= 0) throw std::invalid_argument("CutTube outer radius rmax must be positive!");
    if(dz_ <= 0) throw std::invalid_argument("CutTube half-height dz must be positive!");
    low_norm_ = prepare_normal(low_norm_, false);
    high_norm_ = prepare_normal(high_norm_, true);
    RecomputeWorldAABB();
}
CutTube::CutTube(Placement const & p) : Geometry("CutTube", p), rmin_(0), rmax_(0), dz_(0), low_norm_(math::Vector3D(0, 0, -1)), high_norm_(math::Vector3D(0, 0, 1)), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) { RecomputeWorldAABB(); }
CutTube::CutTube(Placement const & p, double rmin, double rmax, double dz, math::Vector3D low_norm, math::Vector3D high_norm) : Geometry("CutTube", p), rmin_(rmin), rmax_(rmax), dz_(dz), low_norm_(low_norm), high_norm_(high_norm), start_phi_(0), delta_phi_(2.0 * M_PI), has_phi_cut_(false) {
    if(rmin_ < 0) throw std::invalid_argument("CutTube inner radius rmin must be non-negative!");
    if(rmax_ < 0) throw std::invalid_argument("CutTube outer radius rmax must be non-negative!");
    if(rmin_ > rmax_) std::swap(rmin_, rmax_);
    if(rmax_ <= 0) throw std::invalid_argument("CutTube outer radius rmax must be positive!");
...
CutTube::CutTube(double rmin, double rmax, double dz, math::Vector3D low_norm, math::Vector3D high_norm, double start_phi, double delta_phi) : Geometry("CutTube"), rmin_(rmin), rmax_(rmax), dz_(dz), low_norm_(low_norm), high_norm_(high_norm), start_phi_(start_phi), delta_phi_(delta_phi) {
    if(rmin_ < 0) throw std::invalid_argument("CutTube inner radius rmin must be non-negative!");
    if(rmax_ < 0) throw std::invalid_argument("CutTube outer radius rmax must be non-negative!");
    if(rmin_ > rmax_) std::swap(rmin_, rmax_);
    if(rmax_ <= 0) throw std::invalid_argument("CutTube outer radius rmax must be positive!");
...
CutTube::CutTube(Placement const & p, double rmin, double rmax, double dz, math::Vector3D low_norm, math::Vector3D high_norm, double start_phi, double delta_phi) : Geometry("CutTube", p), rmin_(rmin), rmax_(rmax), dz_(dz), low_norm_(low_norm), high_norm_(high_norm), start_phi_(start_phi), delta_phi_(delta_phi) {
    if(rmin_ < 0) throw std::invalid_argument("CutTube inner radius rmin must be non-negative!");
    if(rmax_ < 0) throw std::invalid_argument("CutTube outer radius rmax must be non-negative!");
    if(rmin_ > rmax_) std::swap(rmin_, rmax_);
    if(rmax_ <= 0) throw std::invalid_argument("CutTube outer radius rmax must be positive!");
...
CutTube::CutTube(const CutTube& o) : Geometry(o), rmin_(o.rmin_), rmax_(o.rmax_), dz_(o.dz_), low_norm_(o.low_norm_), high_norm_(o.high_norm_), start_phi_(o.start_phi_), delta_phi_(o.delta_phi_), has_phi_cut_(o.has_phi_cut_) { RecomputeWorldAABB(); }

SIREN_GEOMETRY_SWAP(CutTube, rmin_, rmax_, dz_, low_norm_, high_norm_, start_phi_, delta_phi_, has_phi_cut_)
SIREN_GEOMETRY_ASSIGN(CutTube)
SIREN_GEOMETRY_EQUAL(CutTube, rmin_, rmax_, dz_, low_norm_, high_norm_, start_phi_, delta_phi_, has_phi_cut_)
SIREN_GEOMETRY_LESS(CutTube, rmin_, rmax_, dz_, low_norm_, high_norm_, start_phi_, delta_phi_, has_phi_cut_)

void CutTube::print(std::ostream& os) const {
    os << "CutTube(" << rmin_ << ", " << rmax_ << ", " << dz_ << ", " << low_norm_ << ", " << high_norm_;
...
std::vector<Geometry::Intersection> CutTube::ComputeIntersections(siren::math::Vector3D const & position, siren::math::Vector3D const & direction) const {
    ...
    for(int face = 0; face < 2; ++face) {
        ...
        if(hx*hx + hy*hy < GEOMETRY_PRECISION * 1e3 * GEOMETRY_PRECISION * 1e3) {
            ...
            entering = ZAxisWedgeEntering(dirx, diry, start_phi_, delta_phi_);
        }
    ...
    bool in_wedge = InitialPhiState(px, py, dirx, diry, start_phi_, delta_phi_);
