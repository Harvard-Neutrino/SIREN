...
siren::dataclasses::PhaseSpaceConvention Decay::Convention() const {
    ...
    auto warn_once = [](std::string const & detail) {
        if (!warned.exchange(true)) {
            std::cerr << "Warning: Decay subclass does not override Convention(); "
                      << detail << " Override Convention() to silence this warning."
                      << std::endl;
        }
    };
    ...
    auto signatures = GetPossibleSignatures();
    if (signatures.empty()) {
        warn_once("auto-detected Custom from DensityVariables().");
        return C::Custom;
    }
    size_t n = signatures.front().secondary_types.size();
    for (auto const & sig : signatures) {
        if (sig.secondary_types.size() != n) {
            warn_once("auto-detected Custom (mixed arities) from DensityVariables().");
            return C::Custom;
        }
    }
