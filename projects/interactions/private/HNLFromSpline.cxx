...
HNLFromSpline::HNLFromSpline(std::vector<char> differential_data, std::vector<char> total_data, int interaction, double target_mass, double minimum_Q2, std::set<SI::dataclasses::ParticleType> primary_types, std::set<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types), target_types_(target_types), interaction_type_(interaction), target_mass_(target_mass), minimum_Q2_(minimum_Q2) {
    LoadFromMemory(differential_data, total_data);
    InitializeSignatures();
}

HNLFromSpline::HNLFromSpline(std::vector<char> differential_data, std::vector<char> total_data, int interaction, double target_mass, double minimum_Q2, std::vector<SI::dataclasses::ParticleType> primary_types, std::vector<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types.begin(), primary_types.end()), target_types_(target_types.begin(), target_types.end()), interaction_type_(interaction), target_mass_(target_mass), minimum_Q2_(minimum_Q2) {
    LoadFromMemory(differential_data, total_data);
    InitializeSignatures();
}

HNLFromSpline::HNLFromSpline(std::string differential_filename, std::string total_filename, int interaction, double target_mass, double minimum_Q2, std::set<SI::dataclasses::ParticleType> primary_types, std::set<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types), target_types_(target_types), interaction_type_(interaction), target_mass_(target_mass), minimum_Q2_(minimum_Q2) {
    LoadFromFile(differential_filename, total_filename);
    InitializeSignatures();
}

HNLFromSpline::HNLFromSpline(std::string differential_filename, std::string total_filename, std::set<SI::dataclasses::ParticleType> primary_types, std::set<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types), target_types_(target_types) {
    LoadFromFile(differential_filename, total_filename);
    ReadParamsFromSplineTable();
    InitializeSignatures();
}

HNLFromSpline::HNLFromSpline(std::string differential_filename, std::string total_filename, int interaction, double target_mass, double minimum_Q2, std::vector<SI::dataclasses::ParticleType> primary_types, std::vector<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types.begin(), primary_types.end()), target_types_(target_types.begin(), target_types.end()), interaction_type_(interaction), target_mass_(target_mass), minimum_Q2_(minimum_Q2) {
    LoadFromFile(differential_filename, total_filename);
    InitializeSignatures();
}

HNLFromSpline::HNLFromSpline(std::string differential_filename, std::string total_filename, std::vector<SI::dataclasses::ParticleType> primary_types, std::vector<SI::dataclasses::ParticleType> target_types) : primary_types_(primary_types.begin(), primary_types.end()), target_types_(target_types.begin(), target_types.end()) {
    LoadFromFile(differential_filename, total_filename);
    ReadParamsFromSplineTable();
    InitializeSignatures();
}
...
double HNLFromSpline::GetLeptonMass(SI::dataclasses::ParticleType lepton_type) {
...
double HNLFromSpline::TotalCrossSection(SI::dataclasses::ParticleType primary_type, double primary_energy) const {
...
double HNLFromSpline::TotalCrossSection(SI::dataclasses::ParticleType primary_type, double primary_energy, SI::dataclasses::ParticleType target_type) const {
...
void HNLFromSpline::SampleFinalState(dataclasses::CrossSectionDistributionRecord & record, std::shared_ptr<SI::utilities::LI_random> random) const {
...
std::vector<SI::dataclasses::ParticleType> HNLFromSpline::GetPossiblePrimaries() const {
    return std::vector<SI::dataclasses::ParticleType>(primary_types_.begin(), primary_types_.end());
}

std::vector<SI::dataclasses::ParticleType> HNLFromSpline::GetPossibleTargetsFromPrimary(SI::dataclasses::ParticleType primary_type) const {
...
std::vector<SI::dataclasses::ParticleType> HNLFromSpline::GetPossibleTargets() const {
    return std::vector<SI::dataclasses::ParticleType>(target_types_.begin(), target_types_.end());
}

std::vector<dataclasses::InteractionSignature> HNLFromSpline::GetPossibleSignaturesFromParents(SI::dataclasses::ParticleType primary_type, SI::dataclasses::ParticleType target_type) const {
    std::pair<SI::dataclasses::ParticleType, SI::dataclasses::ParticleType> key(primary_type, target_type);
