...
double ElasticScattering::DifferentialCrossSection(dataclasses::InteractionRecord const & interaction) const {
    ...
    double CLL;
    if(primary_type==SI::dataclasses::ParticleType::NuE) CLL = 0.7276;
    else if(primary_type==SI::dataclasses::ParticleType::NuMu) CLL = -0.2730;
    else {
    ...
    assert(interaction.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuE or interaction.signature.secondary_types[1] == SI::dataclasses::ParticleType::NuE or interaction.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuMu or interaction.signature.secondary_types[1] == SI::dataclasses::ParticleType::NuMu);
    unsigned int nu_index = (interaction.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuE or interaction.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuMu) ? 0 : 1;
    ...
    double term1 = CLL*CLL;// * (1 + SI::utilities::Constants::fineStructure / SI::utilities::Constants::pi * X1);
    double term2 = CLR*CLR * (1-y)*(1-y);// * ( 1 + SI::utilities::Constants::fineStructure / SI::utilities::Constants::pi * X2);
    double term3 = -CLL*CLR*m*y/E;// * (1 + SI::utilities::Constants::fineStructure / SI::utilities::Constants::pi * X3);
    double ret =  std::pow(SI::utilities::Constants::FermiConstant,2) * s / SI::utilities::Constants::pi * (term1 + term2 + term3) / SI::utilities::Constants::invGeVsq_per_cmsq;
    return std::max(ret,0.);
}

// Assume initial electron at rest
double ElasticScattering::DifferentialCrossSection(SI::dataclasses::ParticleType primary_type, double primary_energy, double y) const {
    double CLL;
    if(primary_type==SI::dataclasses::ParticleType::NuE) CLL = 0.7276;
    else if(primary_type==SI::dataclasses::ParticleType::NuMu) CLL = -0.2730;
    else {
    ...
    double m = SI::utilities::Constants::electronMass;
    double E = primary_energy;
    double s = 2*m*E + m*m;
...
double ElasticScattering::TotalCrossSection(SI::dataclasses::ParticleType primary_type, double primary_energy, SI::dataclasses::ParticleType target_type) const {
    double ymax = 2*primary_energy / (2*primary_energy + SI::utilities::Constants::electronMass);
    std::function<double(double)> integrand = [&] (double y) -> double {
        return DifferentialCrossSection(primary_type, primary_energy, y);
    };
    return SI::utilities::rombergIntegrate(integrand, 0, ymax);
}

void ElasticScattering::SampleFinalState(dataclasses::CrossSectionDistributionRecord & record, std::shared_ptr<SI::utilities::LI_random> random) const {

    // Uses Metropolis-Hastings Algorithm!
    // useful for cases where we don't know the supremum of our distribution, and the distribution is multi-dimensional

    SI::dataclasses::ParticleType primary_type = record.signature.primary_type;

    unsigned int nu_index = (record.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuE or record.signature.secondary_types[0] == SI::dataclasses::ParticleType::NuMu) ? 0 : 1;
    unsigned int electron_index = 1 - nu_index;
    ...
    double m3 = SI::utilities::Constants::electronMass;
    ...
    SI::dataclasses::SecondaryParticleRecord & electron = record.GetSecondaryParticleRecord(electron_index);
    SI::dataclasses::SecondaryParticleRecord & neutrino = record.GetSecondaryParticleRecord(nu_index);
...
std::vector<SI::dataclasses::ParticleType> ElasticScattering::GetPossibleTargets() const {
    std::vector<SI::dataclasses::ParticleType> res;
    res.push_back(SI::dataclasses::ParticleType::EMinus);
    return res;
}

std::vector<SI::dataclasses::ParticleType> ElasticScattering::GetPossibleTargetsFromPrimary(SI::dataclasses::ParticleType primary_type) const {
...
std::vector<SI::dataclasses::ParticleType> ElasticScattering::GetPossiblePrimaries() const {
...
std::vector<dataclasses::InteractionSignature> ElasticScattering::GetPossibleSignaturesFromParents(SI::dataclasses::ParticleType primary_type, SI::dataclasses::ParticleType target_type) const {
    std::vector<SI::dataclasses::ParticleType> targets = GetPossibleTargets();
