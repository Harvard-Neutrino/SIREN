...
void InteractionCollection::InitializeTargetTypes() {
    ...
    for(unsigned int i=0; i<cross_sections.size(); ++i) {
        // Gather target types
        std::vector<LI::dataclasses::ParticleType> xs_targets = cross_sections[i]->GetPossibleTargets();
        ...
        for(unsigned int j=0; j<xs_targets.size(); ++j) {
            LI::dataclasses::ParticleType target = xs_targets[j];
            std::map<LI::dataclasses::ParticleType, std::vector<std::shared_ptr<CrossSection>>>::const_iterator it = cross_sections_by_target.find(target);
    ...
    // std::set<LI::dataclasses::ParticleType> target_set(target_types.begin(), target_types.end());
...
InteractionCollection::InteractionCollection(LI::dataclasses::ParticleType primary_type, std::vector<std::shared_ptr<CrossSection>> cross_sections) : primary_type(primary_type), cross_sections(cross_sections) {
    InitializeTargetTypes();
}

InteractionCollection::InteractionCollection(LI::dataclasses::ParticleType primary_type, std::vector<std::shared_ptr<Decay>> decays) : primary_type(primary_type), decays(decays) {
    InitializeTargetTypes();
}

InteractionCollection::InteractionCollection(LI::dataclasses::ParticleType primary_type, std::vector<std::shared_ptr<CrossSection>> cross_sections, std::vector<std::shared_ptr<Decay>> decays) : primary_type(primary_type), cross_sections(cross_sections), decays(decays) {
    InitializeTargetTypes();
}
...
std::vector<std::shared_ptr<CrossSection>> const & InteractionCollection::GetCrossSectionsForTarget(LI::dataclasses::ParticleType p) const {
    std::map<LI::dataclasses::ParticleType, std::vector<std::shared_ptr<CrossSection>>>::const_iterator it = cross_sections_by_target.find(p);
    if(it != cross_sections_by_target.end()) {
        return it->second;
    } else {
        return empty;
