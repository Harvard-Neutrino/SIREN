...
void register_InteractionCollection(pybind11::module_ & m) {
    ...
    class_<InteractionCollection, std::shared_ptr<InteractionCollection>>(m, "InteractionCollection")
        .def(init<>())
        .def(init<siren::dataclasses::ParticleType, std::vector<std::shared_ptr<CrossSection>>>(), keep_alive<1, 3>())
        .def(init<siren::dataclasses::ParticleType, std::vector<std::shared_ptr<Decay>>>(), keep_alive<1, 3>())
        .def(init<siren::dataclasses::ParticleType, std::vector<std::shared_ptr<CrossSection>>, std::vector<std::shared_ptr<Decay>>>(), keep_alive<1, 3>(), keep_alive<1, 4>())
        .def(init<siren::dataclasses::ParticleType, std::vector<std::shared_ptr<Interaction>>>(), keep_alive<1, 3>())
        .def(self == self)
