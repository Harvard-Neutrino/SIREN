...
double DipoleFromTable::TotalCrossSection(siren::dataclasses::ParticleType primary_type, double primary_energy, siren::dataclasses::ParticleType target_type) const {
    ...
    siren::utilities::Interpolator1D<double> const & interp = total.at(target_type);
    ...
    siren::utilities::Interpolator1D<double> const & interp_proton = total.at(siren::dataclasses::ParticleType::HNucleus);
    int nprotons = siren::detector::MaterialModel::GetProtonCount(target_type);
    if(!inelastic || target_type==siren::dataclasses::ParticleType::HNucleus) {
        nprotons = 0;
    }
    ...
    if(in_invGeV)
        return std::pow(dipole_coupling, 2) * xsec / siren::utilities::Constants::invGeVsq_per_cmsq;
    else
...
double DipoleFromTable::DifferentialCrossSection(siren::dataclasses::ParticleType primary_type, double primary_energy, siren::dataclasses::ParticleType target_type, double target_mass, double y, double thresh) const {
    ...
    siren::utilities::Interpolator2D<double> const & interp_proton = differential.at(siren::dataclasses::ParticleType::HNucleus);
    ...
    if(in_invGeV)
        differential_cross_section /= siren::utilities::Constants::invGeVsq_per_cmsq;
...
void DipoleFromTable::SampleFinalState(dataclasses::CrossSectionDistributionRecord & record, std::shared_ptr<siren::utilities::LI_random> random) const {
    ...
    if(!inelastic || record.signature.target_type == siren::dataclasses::ParticleType::HNucleus) {
        nprotons = 0;
    }
    ...
    if(primary_energy < thresh) {
        throw(siren::utilities::InjectionFailure("Primary is below interaction threshold!"));
    }
...
void DipoleFromTable::AddDifferentialCrossSectionFile(std::string filename, siren::dataclasses::ParticleType target) {
    ...
    siren::dataclasses::ParticleType pid = (siren::dataclasses::ParticleType)pid_int;
    ...
    if(fexists(filename)) {
        ...
        siren::utilities::Interpolator2D<double> interp(table_data);
...
void DipoleFromTable::AddTotalCrossSectionFile(std::string filename, siren::dataclasses::ParticleType target) {
    ...
    if(fexists(filename)) {
        ...
        siren::utilities::Interpolator1D<double> interp(table_data);
