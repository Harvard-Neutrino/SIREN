...
#include "SIREN/utilities/Constants.h"            // for invGeVsq_per_cmsq
...
HNLDipoleDISFromSpline::HNLDipoleDISFromSpline(std::string differential_filename, std::string total_filename, double hnl_mass, std::vector<double> dipole_coupling, std::vector<siren::dataclasses::ParticleType> primary_types, std::vector<siren::dataclasses::ParticleType> target_types, std::string units) : hnl_mass_(hnl_mass), dipole_coupling_(dipole_coupling), primary_types_(primary_types.begin(), primary_types.end()), target_types_(target_types.begin(), target_types.end()) {
    LoadFromFile(differential_filename, total_filename);
    ReadParamsFromSplineTable();
    InitializeSignatures();
    SetUnits(units);
...
bool HNLDipoleDISFromSpline::equal(CrossSection const & other) const {
    const HNLDipoleDISFromSpline* x = dynamic_cast<const HNLDipoleDISFromSpline*>(&other);
...
void HNLDipoleDISFromSpline::LoadFromFile(std::string dd_crossSectionFile, std::string total_crossSectionFile) {
...
void HNLDipoleDISFromSpline::InitializeSignatures() {
    signatures_.clear();
    for(auto primary_type : primary_types_) {
        ...
        signature.secondary_types.push_back(siren::dataclasses::Particle::ParticleType::Hadrons);
        for(auto target_type : target_types_) {
            signature.target_type = target_type;

            signatures_.push_back(signature);

            std::pair<siren::dataclasses::Particle::ParticleType, siren::dataclasses::Particle::ParticleType> key(primary_type, target_type);
            signatures_by_parent_types_[key].push_back(signature);
        }
    }
}

double HNLDipoleDISFromSpline::TotalCrossSection(dataclasses::InteractionRecord const & interaction) const {
...
double HNLDipoleDISFromSpline::TotalCrossSection(siren::dataclasses::Particle::ParticleType primary_type, double primary_energy) const {
...
double HNLDipoleDISFromSpline::DifferentialCrossSection(dataclasses::InteractionRecord const & interaction) const {
    rk::P4 p1(geom3::Vector3(interaction.primary_momentum[1], interaction.primary_momentum[2], interaction.primary_momentum[3]), interaction.primary_mass);
...
double HNLDipoleDISFromSpline::DifferentialCrossSection(siren::dataclasses::Particle::ParticleType primary_type, double energy, double x, double y, double Q2) const {
...
void HNLDipoleDISFromSpline::SampleFinalState(dataclasses::CrossSectionDistributionRecord& interaction, std::shared_ptr<siren::utilities::SIREN_random> random) const {
    ...
    siren::dataclasses::SecondaryParticleRecord & hnl = secondaries[hnl_index];
...
std::vector<siren::dataclasses::Particle::ParticleType> HNLDipoleDISFromSpline::GetPossibleTargetsFromPrimary(siren::dataclasses::Particle::ParticleType primary_type) const {
    return std::vector<siren::dataclasses::Particle::ParticleType>(target_types_.begin(), target_types_.end());
}

std::vector<dataclasses::InteractionSignature> HNLDipoleDISFromSpline::GetPossibleSignatures() const {
    return std::vector<dataclasses::InteractionSignature>(signatures_.begin(), signatures_.end());
}

std::vector<siren::dataclasses::Particle::ParticleType> HNLDipoleDISFromSpline::GetPossibleTargets() const {
    return std::vector<siren::dataclasses::Particle::ParticleType>(target_types_.begin(), target_types_.end());
}

std::vector<dataclasses::InteractionSignature> HNLDipoleDISFromSpline::GetPossibleSignaturesFromParents(siren::dataclasses::Particle::ParticleType primary_type, siren::dataclasses::Particle::ParticleType target_type) const {
    std::pair<siren::dataclasses::Particle::ParticleType, siren::dataclasses::Particle::ParticleType> key(primary_type, target_type);
...
std::vector<std::string> HNLDipoleDISFromSpline::DensityVariables() const {
