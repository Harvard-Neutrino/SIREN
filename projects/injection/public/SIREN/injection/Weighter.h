...
private:
    ...
    std::vector<std::shared_ptr<SI::distributions::WeightableDistribution>> physical_distributions;

    std::vector<std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>> unique_distributions;
...
public:
    ...
    double InteractionProbability(std::shared_ptr<Injector const> injector, SI::dataclasses::InteractionRecord const & record) const;
    double InteractionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const;
    double UnnormalizedPositionProbability(std::shared_ptr<Injector const> injector, SI::dataclasses::InteractionRecord const & record) const;
    double UnnormalizedPositionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const;
    double NormalizedPositionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const;
    ...
    LeptonWeighter(std::vector<std::shared_ptr<Injector>> injectors, std::shared_ptr<SI::detector::DetectorModel> detector_model, std::shared_ptr<SI::interactions::InteractionCollection> interactions, std::vector<std::shared_ptr<SI::distributions::WeightableDistribution>> physical_distributions);
    double EventWeight(SI::dataclasses::InteractionRecord const & record) const;
    double SimplifiedEventWeight(SI::dataclasses::InteractionRecord const & record) const;
};

} //namespace injection
} //namespace SI

CEREAL_CLASS_VERSION(SI::injection::LeptonWeighter, 0);
