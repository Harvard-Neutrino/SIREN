...
protected:
    ...
    std::shared_ptr<SI::distributions::RangePositionDistribution> position_distribution;
    std::shared_ptr<SI::interactions::InteractionCollection> interactions;
    RangedSIREN();
public:
    RangedSIREN(unsigned int events_to_inject, std::shared_ptr<SI::detector::DetectorModel> detector_model, std::shared_ptr<injection::PrimaryInjectionProcess> primary_process, std::vector<std::shared_ptr<injection::SecondaryInjectionProcess>> secondary_processes, std::shared_ptr<SI::utilities::LI_random> random, std::shared_ptr<SI::distributions::RangeFunction> range_func, double disk_radius, double endcap_length);
...
CEREAL_CLASS_VERSION(SI::injection::RangedSIREN, 0);
CEREAL_REGISTER_TYPE(SI::injection::RangedSIREN);
CEREAL_REGISTER_POLYMORPHIC_RELATION(SI::injection::Injector, SI::injection::RangedSIREN);
