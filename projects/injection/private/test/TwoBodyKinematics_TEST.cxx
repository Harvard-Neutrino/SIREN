...
TEST(TwoBodyKinematics, JacobianConsistency) {
    ...
    for (auto & sol : solutions) {
        if (sol.valid && std::abs(sol.cos_theta_rest - cos_rest) < 1e-5) {
            // The Jacobian is dOmega_lab/dOmega_rest. Since phi is unchanged under a
            // boost along z, this equals |d(cos_theta_lab)/d(cos_theta_rest)|, which is
            // exactly what sol.jacobian reports.
