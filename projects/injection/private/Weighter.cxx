...
double LeptonWeighter::InteractionProbability(std::shared_ptr<Injector const> injector, SI::dataclasses::InteractionRecord const & record) const {
    std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds = injector->PrimaryInjectionBounds(record);
    return InteractionProbability(bounds, record);
}

double LeptonWeighter::InteractionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const {
    ...
    std::map<SI::dataclasses::ParticleType, std::vector<std::shared_ptr<SI::interactions::CrossSection>>> const & cross_sections_by_target = interactions->GetCrossSectionsByTarget();
...
double LeptonWeighter::UnnormalizedPositionProbability(std::shared_ptr<Injector const> injector, SI::dataclasses::InteractionRecord const & record) const {
    std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds = injector->PrimaryInjectionBounds(record);
    return UnnormalizedPositionProbability(bounds, record);
}

double LeptonWeighter::UnnormalizedPositionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const {
    SI::math::Vector3D interaction_vertex(
            record.interaction_vertex[0],
            record.interaction_vertex[1],
            record.interaction_vertex[2]);
...
double LeptonWeighter::NormalizedPositionProbability(std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds, SI::dataclasses::InteractionRecord const & record) const {
    SI::math::Vector3D interaction_vertex(
            record.interaction_vertex[0],
            record.interaction_vertex[1],
            record.interaction_vertex[2]);
...
void LeptonWeighter::Initialize() {
    ...
    std::vector<std::pair<bool, std::shared_ptr<SI::distributions::WeightableDistribution>>> physical_init_state;
    ...
    std::vector<std::vector<std::pair<bool, std::shared_ptr<SI::distributions::WeightableDistribution>>>> physical_distribution_state(injectors.size(), physical_init_state);
    assert(physical_distribution_state.size() == injectors.size());
    // Initialize the state for generation distributions
    // true ==> distribution does not cancel and is not common
    std::vector<std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>>> generation_distribution_state;
    generation_distribution_state.reserve(injectors.size());
    for(auto injector : injectors) {
        std::vector<std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>> dists = injector->GetPrimaryInjectionDistributions();
        std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>> dist_state;
    ...
    for(unsigned int i=0; i<injectors.size(); ++i) {
        ...
        std::vector<std::pair<bool, std::shared_ptr<SI::distributions::WeightableDistribution>>> & phys_dists = physical_distribution_state[i];
        std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>> & gen_dists = generation_distribution_state[i];
        ...
        for(unsigned int phys_idx=0; phys_idx<phys_dists.size(); ++phys_idx) {
            std::pair<bool, std::shared_ptr<SI::distributions::WeightableDistribution>> & phys_dist = phys_dists[phys_idx];
            if(not phys_dist.first) // Skip if already cancelled
                continue;
            for(unsigned int gen_idx=0; gen_idx<gen_dists.size(); ++gen_idx) {
                std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>> & gen_dist = gen_dists[gen_idx];
                ...
                std::shared_ptr<SI::distributions::WeightableDistribution> gen_dist_ptr(gen_dist.second);
    ...
    // Physical distributions have the same DetectorModel+CrossSection context so we do not need to compare them
    ...
    for(unsigned int phys_idx=0; phys_idx<physical_distributions.size(); ++phys_idx) {
        ...
        if(dynamic_cast<const SI::distributions::VertexPositionDistribution*>(physical_distributions[phys_idx].get())) {
    ...
    unsigned int i=0;
    std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>> & gen_dists_0 = generation_distribution_state[i];
    for(unsigned int gen_idx_0=0; gen_idx_0<gen_dists_0.size(); ++gen_idx_0) {
        std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>> & gen_dist_0 = gen_dists_0[gen_idx_0];
        ...
        for(unsigned int j=i+1; j<injectors.size(); ++j) {
            ...
            std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>> & gen_dists_1 = generation_distribution_state[j];
            for(unsigned int gen_idx_1=0; gen_idx_1<gen_dists_1.size(); ++gen_idx_1) {
                std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>> & gen_dist_1 = gen_dists_1[gen_idx_1];
                ...
                bool equivalent_dists =
                    gen_dist_0.second->AreEquivalent( // gen dist 0
                            ...
                            (std::shared_ptr<SI::distributions::WeightableDistribution>)(gen_dist_1.second), // gen dist 1
    ...
    for(unsigned int gen_idx : common_generation_dist_idxs) {
        ...
        std::shared_ptr<SI::distributions::WeightableDistribution> dist = generation_distribution_state[0][gen_idx].second;
        std::shared_ptr<SI::detector::DetectorModel> dist_detector = injectors[0]->GetDetectorModel();
        std::shared_ptr<SI::interactions::InteractionCollection> dist_interactions = injectors[0]->GetInteractions();
        std::function<bool(std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>)> predicate = [&] (std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>> p) -> bool {
    ...
    for(unsigned int phys_idx : common_physical_dist_idxs) {
        std::shared_ptr<SI::distributions::WeightableDistribution> dist = physical_distributions[phys_idx];
        std::function<bool(std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>)> predicate = [&] (std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>> p) -> bool {
    ...
    for(unsigned int injector_idx=0; injector_idx<injectors.size(); ++injector_idx) {
        std::vector<std::pair<bool, std::shared_ptr<SI::distributions::WeightableDistribution>>> & phys_dists = physical_distribution_state[injector_idx];
        std::vector<std::pair<bool, std::shared_ptr<SI::distributions::PrimaryInjectionDistribution>>> & gen_dists = generation_distribution_state[injector_idx];
        ...
        for(unsigned int gen_idx=0; gen_idx<gen_dists.size(); ++gen_idx) {
            ...
            if(not included)
                continue;
            std::shared_ptr<SI::distributions::WeightableDistribution> dist = gen_dists[gen_idx].second;
            // These are common to all injectors, so we pull information from the first injector
            std::shared_ptr<SI::detector::DetectorModel> dist_detector = injectors[injector_idx]->GetDetectorModel();
            std::shared_ptr<SI::interactions::InteractionCollection> dist_interactions = injectors[injector_idx]->GetInteractions();
            std::function<bool(std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>)> predicate = [&] (std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>> p) -> bool {
                return std::get<0>(p)->AreEquivalent(std::get<1>(p), std::get<2>(p), dist, dist_detector, dist_interactions);
            };
            auto it = std::find_if(unique_distributions.begin(), unique_distributions.end(), predicate);
        ...
        for(unsigned int phys_idx=0; phys_idx<phys_dists.size(); ++phys_idx) {
            ...
            if(not included)
                continue;
            std::shared_ptr<SI::distributions::WeightableDistribution> dist = phys_dists[phys_idx].second;
            std::function<bool(std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>)> predicate = [&] (std::tuple<std::shared_ptr<SI::distributions::WeightableDistribution>, std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>> p) -> bool {
                return std::get<0>(p)->AreEquivalent(std::get<1>(p), std::get<2>(p), dist, detector_model, interactions);
            };
            auto it = std::find_if(unique_distributions.begin(), unique_distributions.end(), predicate);
    ...
    // std::vector<std::tuple<std::shared_ptr<SI::detector::DetectorModel>, std::shared_ptr<SI::interactions::InteractionCollection>>> unique_contexts;
    // std::vector<unsigned int> context_idx_by_injector;
}

LeptonWeighter::LeptonWeighter(std::vector<std::shared_ptr<Injector>> injectors, std::shared_ptr<SI::detector::DetectorModel> detector_model, std::shared_ptr<SI::interactions::InteractionCollection> interactions, std::vector<std::shared_ptr<SI::distributions::WeightableDistribution>> physical_distributions)
    : injectors(injectors)
...
double LeptonWeighter::EventWeight(SI::dataclasses::InteractionRecord const & record) const {
    ...
    for(auto injector : injectors) {
        ...
        */
        ...
        physical_probability *= prob;
        prob = SI::injection::CrossSectionProbability(injector->GetDetectorModel(), injector->GetInteractions(), record);
        physical_probability *= prob;
...
double LeptonWeighter::SimplifiedEventWeight(SI::dataclasses::InteractionRecord const & record) const {
    std::vector<double> probs;
    probs.reserve(unique_distributions.size());
    for(unsigned int i=0; i<unique_distributions.size(); ++i) {
        std::tuple<
            std::shared_ptr<SI::distributions::WeightableDistribution>,
            std::shared_ptr<SI::detector::DetectorModel>,
            std::shared_ptr<SI::interactions::InteractionCollection>
    ...
    double prob = SI::injection::CrossSectionProbability(detector_model, interactions, record);
    phys_over_gen *= prob;
    ...
    for(unsigned int i=0; i<injectors.size(); ++i) {
        ...
        double cross_section_probability = SI::injection::CrossSectionProbability(injectors[i]->GetDetectorModel(), injectors[i]->GetInteractions(), record);
        ...
        std::tuple<SI::math::Vector3D, SI::math::Vector3D> bounds = injectors[i]->PrimaryInjectionBounds(record);
