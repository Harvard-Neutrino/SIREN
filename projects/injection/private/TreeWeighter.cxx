...
void LeptonTreeWeighter::Initialize() {
  ...
  for(auto const & injector : injectors) {
    assert(injector->GetPrimaryProcess() == primary_physical_process);
    ...
    for(auto const & sec_phys_process : secondary_physical_processes) {
      try{
        ...
        assert(sec_phys_process==sec_inj_process); // make sure cross section collection matches
    ...
    if(injector_sec_process_weigter_map.size() != injector_sec_process_map.size()) {
...
double LeptonTreeWeighter::EventWeight(LI::dataclasses::InteractionTree const & tree) const {
  ...
  //
  double inv_weight = 0;
  for(int idx = 0; idx < injectors.size(); ++idx) {
    ...
    for(auto const datum : tree.tree) {
      ...
      if(datum->depth()==0) {
        ...
        try {
          double phys_prob = secondary_process_weighter_maps[idx][datum->record.signature.primary_type]->PhysicalProbability(bounds, datum->record);
...
void LeptonProcessWeighter::Initialization() {
  normaliztion = 1.0;
...
double LeptonProcessWeighter::EventWeight(LI::dataclasses::InteractionRecord const & record,
                                          std::pair<LI::math::Vector3D, LI::math::Vector3D> bounds) const {
}
