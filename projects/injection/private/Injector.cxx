    ...
    std::shared_ptr<siren::interactions::InteractionCollection> interactions) const
{
    siren::dataclasses::CrossSectionDistributionRecord xsec_record(record);
    bool sampled = false;
    // Sample the final state, then let the selected interaction override the
    // vertex time (post-selection, before Finalize back-syncs daughters).
        ...
        for (auto const & xs : interactions->GetCrossSectionsForTarget(record.signature.target_type)) {
            for (auto const & sig : xs->GetPossibleSignaturesFromParents(record.signature.primary_type, record.signature.target_type)) {
                if (sig == record.signature) {
                    ...
                    proposed_time = xs->SampleInteractionTime(xsec_record, random);
                    sampled = true;
                    break;
                }
            }
    ...
    std::function<void(std::shared_ptr<siren::dataclasses::InteractionTreeDatum>)> add_secondaries = [&](std::shared_ptr<siren::dataclasses::InteractionTreeDatum> parent) {
        for(size_t i=0; i<parent->record.signature.secondary_types.size(); ++i) {
            ...
            if(it == secondary_process_map.end()) {
                ...
                int depth = static_cast<int>(parent->depth(tree)) + 1;
                std::ostringstream oss;
                oss << "secondary type not registered: " << type;
                failure_ledger_.Record(depth, parent_pdg,
                    siren::utilities::FailureReason::UnregisteredSecondaryType, oss.str());
                continue;
            }
