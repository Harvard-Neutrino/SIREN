...
std::tuple<SI::math::Vector3D, SI::math::Vector3D> PointSourcePositionDistribution::SamplePosition(std::shared_ptr<SI::utilities::LI_random> rand, std::shared_ptr<SI::detector::DetectorModel const> detector_model, std::shared_ptr<SI::interactions::InteractionCollection const> interactions, SI::dataclasses::PrimaryDistributionRecord & record) const {
    SI::math::Vector3D dir(record.GetDirection());

    SI::math::Vector3D endcap_0 = origin;
    SI::math::Vector3D endcap_1 = origin + max_distance * dir;

    SI::detector::Path path(detector_model, DetectorPosition(endcap_0), DetectorDirection(dir), max_distance);
...
double PointSourcePositionDistribution::GenerationProbability(std::shared_ptr<SI::detector::DetectorModel const> detector_model, std::shared_ptr<SI::interactions::InteractionCollection const> interactions, SI::dataclasses::InteractionRecord const & record) const {
...
PointSourcePositionDistribution::PointSourcePositionDistribution(SI::math::Vector3D origin, double max_distance, std::set<SI::dataclasses::ParticleType> target_types) : origin(origin), max_distance(max_distance), target_types(target_types) {}
...
std::tuple<SI::math::Vector3D, SI::math::Vector3D> PointSourcePositionDistribution::InjectionBounds(std::shared_ptr<SI::detector::DetectorModel const> detector_model, std::shared_ptr<SI::interactions::InteractionCollection const> interactions, SI::dataclasses::InteractionRecord const & record) const {
    SI::math::Vector3D dir(record.primary_momentum[1], record.primary_momentum[2], record.primary_momentum[3]);
    dir.normalize();
    SI::math::Vector3D vertex(record.interaction_vertex); // m
